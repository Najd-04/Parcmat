...
@section('content')
    <div class="container">
        <h1 class="mb-4">Nouvelle Machine</h1>
                ...
                <form action="{{ route('machines.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('modele_id') is-invalid @enderror"
                            ...
                            @foreach($modeles as $modele)
                                <option value="{{ $modele->id }}" {{ old('modele_id') == $modele->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type_machine_id') is-invalid @enderror"
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type->id }}" {{ old('type_machine_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('detail_appareil') is-invalid @enderror"
                                  id="detail_appareil" name="detail_appareil" rows="3">{{ old('detail_appareil') }}</textarea>
